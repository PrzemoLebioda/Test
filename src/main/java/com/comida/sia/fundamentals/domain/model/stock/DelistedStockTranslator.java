...
public class DelistedStockTranslator implements ModelTranslator<StockData, Stock> {
	private Notifier notifier;
	
	public DelistedStockTranslator(Notifier notifier) {
	...
	public Stock translate(StockData source) throws ParseException {
		if(isTranslatable(source)) {
			Stock stock = new Stock(source.getSymbol(), source.getName(), TranslationConcern.getDateFrom(source.getIpoDate()));
			stock.setAssetType(AssetType.get(source.getAssetType()));
			stock
				.withNotifier(notifier)
				.delist(TranslationConcern.getDateFrom(source.getDelistingDate()));
	...
	private Boolean isTranslatable(StockData source) {
		if(hasSymbol(source) && hasDelistedDate(source)) {
			return true;
		} else {
			return false;
		}
	...
	private Boolean hasSymbol(StockData source) {
	...
	private Boolean hasDelistedDate(StockData source) {
		if(source.getDelistingDate() != null &&  !source.getDelistingDate().isEmpty()) {
			return true;
		} else {
			return false;
		}
