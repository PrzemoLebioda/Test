...
public class SynchronizeListedStockCommand extends Command<StockService> {

	private SynchronizationStateDto syncState;
	
	public SynchronizeListedStockCommand(StockService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeListedStock(syncState);
	}
}