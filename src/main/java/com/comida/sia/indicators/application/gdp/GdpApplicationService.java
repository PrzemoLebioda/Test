...
@Slf4j
@Component("GdpApplicationService")
public class GdpApplicationService implements GdpService, Notifier{

	@Autowired
	@Qualifier("AlfavantageGdpDataAcquirer")
	...
	@Autowired
	@Qualifier("GdpHibernateRepository")
	public GdpRepository repository;
	...
	public void synchronizeAnnualGdp(SynchronizationStateDto syncState) {
		try {
			...
			persist(
					assembler
						.assemblyGdpAnnualSyncAdmin(syncState, this)
						.synchronize(annualGdpData.getData(), syncState)
			);
			
		} catch (EmptyListException e){
			log.warn("GDP annual list is empty");
		} catch (Exception e) {
	...
	public void synchronizeQuarterlyGdp(SynchronizationStateDto syncState) {
		try {
			...
			persist(
					assembler
						.assemblyGdpQuarterSyncAdmin(syncState, this)
						.synchronize(quarterlyGdpData.getData(), syncState)
			);
			
		} catch (EmptyListException e){
			log.warn("GDP quarterly list is empty");
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	}

	private void persist(List<GrossDomesticProduct> gdpList) {
		for(GrossDomesticProduct gdp : gdpList) {
			persist(gdp);
		}
	}

	private void persist(GrossDomesticProduct gdp) {
		try {
