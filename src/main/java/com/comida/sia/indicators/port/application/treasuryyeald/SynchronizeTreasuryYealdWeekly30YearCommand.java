...
public class SynchronizeTreasuryYealdWeekly30YearCommand  extends Command<TreasuryYealdService>{

	private SynchronizationStateDto syncState;
	
	public SynchronizeTreasuryYealdWeekly30YearCommand(TreasuryYealdService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeTreasuryYealdWeekly30Year(syncState);
	}
