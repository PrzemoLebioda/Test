...
public class SynchronizeTreasuryYealdDaily03MonthCommand extends Command<TreasuryYealdService> {

	private SynchronizationStateDto syncState;
	
	public SynchronizeTreasuryYealdDaily03MonthCommand(TreasuryYealdService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeTreasuryYealdDaily03Month(syncState);
	}
