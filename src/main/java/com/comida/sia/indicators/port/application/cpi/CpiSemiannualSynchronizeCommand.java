...
public class CpiSemiannualSynchronizeCommand extends Command<CpiService>{

	private SynchronizationStateDto syncState;
	
	public CpiSemiannualSynchronizeCommand(CpiService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeSemiannualCpi(syncState);
	}
