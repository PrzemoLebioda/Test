...
public class SynchronizeMonthlyIntrestRateCommand extends Command<IntrestRateService> {

	private SynchronizationStateDto syncState;
	
	public SynchronizeMonthlyIntrestRateCommand(IntrestRateService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeMonthlyIntrestRate(syncState);
	}
}
