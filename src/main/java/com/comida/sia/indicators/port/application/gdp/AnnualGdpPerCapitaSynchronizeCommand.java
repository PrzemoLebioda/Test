...
public class AnnualGdpPerCapitaSynchronizeCommand extends Command<GdpService>{
	
	private SynchronizationStateDto syncState;
	
	public AnnualGdpPerCapitaSynchronizeCommand(GdpService requestClass, SynchronizationStateDto syncState) {
		super(requestClass);
	...
	public void execute() {
		AssertionConcern.assertNotNull(syncState, "Current synchronization state is mandatory in order to execute command");
		requestClass.synchronizeAnnualGdp(syncState);
	}
}
