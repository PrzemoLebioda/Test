...
@Slf4j
@Component("AlfavantageGdpDataAcquirer")
@PropertySource({"classpath:application.properties"})
public class AlfavantageGdpDataAcquirer implements IndicatorsDataAcquirer {
	...
	private static String FUNCTION_NAME = "REAL_GDP";
	...
	public IndicatorsData gatherIndicatorData(Interval interval) throws IOException {
		...
		if(cacheFileManager.canReadCashe()) {
			...
			log.info("Gross Domestic Product for " + interval + " interval read from cashe");
		} else {
			...
			log.info("Gross Domestic Product for " + interval + " interval read from alfavantage");
		}
	...
	private CacheFileManager getCacheFileManager(Interval interval) {
		...
		String fileName = interval.name() + "_us_gdp";
