...
public class NewsFeedSyncDomainEventAnnouncer extends SyncDomainEventAnnouncer {

	public NewsFeedSyncDomainEventAnnouncer(Notifier notifier, SynchronizationState syncState) {
	...
	private SubjectedPayload getDomainEvent() {
		switch(syncState.getScope()) {
			case NEWS_FEED:
				return new NewsFeedSynchronizationOrderedDomainEvent(new SynchronizationStateDto(syncState));
			default:
