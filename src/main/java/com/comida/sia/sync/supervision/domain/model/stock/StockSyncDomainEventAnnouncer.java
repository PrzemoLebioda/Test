...
public class StockSyncDomainEventAnnouncer extends SyncDomainEventAnnouncer {

	public StockSyncDomainEventAnnouncer(Notifier notifier, SynchronizationState syncState) {
	...
	private SubjectedPayload getDomainEvent() {		
		switch(syncState.getScope()) {
			case LISTING_SYNC:
				return new ListedStockSynchronizationOrderedDomainEvent(new SynchronizationStateDto(syncState));
			case DELISTING_SYNC:
				return new DelistedStockSynchronizationOrderedDomainEvent(new SynchronizationStateDto(syncState));
			default:
