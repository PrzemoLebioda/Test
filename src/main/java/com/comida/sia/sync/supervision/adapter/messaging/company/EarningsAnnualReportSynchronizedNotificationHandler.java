...
import com.comida.sia.fundamentals.port.messaging.earnings.EarningsAnnualReportSynchronizedNotification;
...
@Component("EarningsAnnualReportSynchronizedNotificationHandler")
public class EarningsAnnualReportSynchronizedNotificationHandler implements NotificationHandler {

	private EarningsAnnualReportSynchronizedNotification notification;
	...
	public EarningsAnnualReportSynchronizedNotificationHandler(
	...
	public boolean canHandle(String message) {
		try {
			notification = NotificationSerializer
					.instance()
					.deserialize(message, EarningsAnnualReportSynchronizedNotification.class);
			
			if(Subject.EARNINGS_ANNUAL_REPORT_SYNCED.equals(notification.getSubject())) {
				return true;
			} else {
				return false;
			}
	...
	private Command<CompanySynchronizationSupervisorService> buildCommand(EarningsAnnualReportSynchronizedDomainEvent domainEvent) {
