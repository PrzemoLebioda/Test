...
import com.comida.sia.indicators.port.messaging.gdp.GdpAnnualSynchronizedNotification;
...
@Component("GdpAnnualSynchronizedNotificationHandler")
public class GdpAnnualSynchronizedNotificationHandler implements NotificationHandler{

	private GdpAnnualSynchronizedNotification notification;
	...
	public GdpAnnualSynchronizedNotificationHandler(
	...
	public boolean canHandle(String message) {
		try {
			notification = NotificationSerializer
					.instance()
					.deserialize(message, GdpAnnualSynchronizedNotification.class);
			
			if(Subject.GDP_ANNUAL_SYNCED.equals(notification.getSubject())) {
				return true;
			} else {
				return false;
			}
	...
	private Command<IndicatorsSynchronizationSupervisorService> buildCommand(GdpAnnualSynchronizedDomainEvent domainEvent) {
