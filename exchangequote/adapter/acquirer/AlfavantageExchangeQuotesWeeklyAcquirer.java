...
@Slf4j
@Component("AlfavantageExchangeQuotesWeeklyAcquirer")
@PropertySource({"classpath:application.properties"})
public class AlfavantageExchangeQuotesWeeklyAcquirer implements ExchangeQuotesAcquirer {
	...
	private static String FUNCTION = "TIME_SERIES_WEEKLY_ADJUSTED";
	...
	public List<ExchangeQuotationEntry> gatherExchangeQuoteFor(String symbol) throws IOException, CsvException, ParseException {
		CacheFileManager cacheFileManager = getCacheFileManager(symbol, "weekly_adjusted");
		
		if(cacheFileManager.canReadCashe()) {
			log.info("Exchange quotes for " + symbol + ": weekly adjusted gathered from cache");
			return getExchangeQuotesEntryList(cacheFileManager.readFile());
		} else {
			log.info("Exchange quotes for " + symbol + ": weekly adjusted gathered from alfavantage");
	...
	private CacheFileManager getCacheFileManager(String tickerSymbol, String name) {
		...
		return new CacheFileManager(relativePath, fileName, FileType.CSV, PeriodType.WEEKLY);
	}
